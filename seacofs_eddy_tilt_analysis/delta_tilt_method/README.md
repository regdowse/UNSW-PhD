...
## Projected extent versus tilt over eddy lifetimes

`max_projected_extent_vs_tilt_lifetime.ipynb` uses **saved `surface.TiltDis`** from `tilt.load_tilt_tables(paths, add_regions=True, grid=grid)`. Only maximum projected extent is calculated from each day's vertical centres, using saved `TiltDir` and undoing its configured bearing offset. No delta tilt is recalculated. Set `N_EDDIES`, `SEED`, or `EDDY_IDS`; your current selection settings are retained. Lifetime plots preserve saved surface values even when vertical data are absent, and show signed differences where both measures exist. `SAVE=True` writes PNG and CSV files to `extent_lifetime_outputs/`.
