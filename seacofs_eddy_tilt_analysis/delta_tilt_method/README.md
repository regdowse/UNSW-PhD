...
`max_projected_extent_vs_tilt_lifetime.ipynb` selects two random eddies by default and plots maximum projected extent and delta `TiltDis` through each available lifetime, with a signed-difference panel underneath. Set `N_EDDIES`, change `SEED`, or provide `EDDY_IDS`. Both distances use the same reference-depth-limited five-day Gaussian fit as the schematic; they are recomputed from vertical profiles. This compares projected extent, not the pairwise 2-D span used in the older sensitivity notebook. Missing calendar days remain gaps. `SAVE=True` writes PNG and CSV outputs to `extent_lifetime_outputs/`.
